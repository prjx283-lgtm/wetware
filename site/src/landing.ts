...
function hero(): void {
  ...
  function frame(): void {
    ...
    // Body path: a slow drift across the upper right, crawling wave along it.
    const len = Math.min(w * 0.42, 560);
    const cx = w * 0.66 + Math.sin(t * 0.07) * w * 0.05;
    const cy = h * 0.40 + Math.cos(t * 0.05) * h * 0.06;
